...
These parameters come in the form *"dimN"* (e.g. dim1, dim2, ...), up to the number of available dimensions for the selected indicator (use `ineptR::get_dim_info()` to see the available dimensions).
...
```{r}
indicator_metadata <- ineptR::get_metadata(indicator, lang = "EN")
indicator_metadata
...
The `ineptR::get_ine_data()` function conveniently allows us to only request the desired data:
```{r}
...
The parameters passed to `ineptR::get_ine_data()` are a subset from the **categ_cod** values that we obtained with the `ineptR::get_dim_values()`.  
...
```{r}
indicator <- "0008206"
ineptR::get_dim_info(indicator, lang = "EN")

#Check number of unique values by dimension
unique_values <- ineptR::get_dim_values(indicator, lang = "EN") %>% 
  group_by(dim_num) %>% 
